...
# Tarjima qilingan matn mavjudligini tekshirish
            if translated_text:
                ...
                try:
                    ...
                    if message.media:
                        # Agar media bo'lsa va matn Telegram limitidan (1024) uzun bo'lsa
                        if len(translated_text) > 1024:
                            print("ℹ️ Matn uzun. Media alohida, matn alohida yuboriladi.")
                            # Avval mediani o'zini yuboramiz
                            await client.send_file(TARGET_CHANNEL, message.media)
                            # Keyin uzun matnni alohida xabar qilib yuboramiz
